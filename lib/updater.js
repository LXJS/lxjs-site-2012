...
var PUBLIC_PATH = '/home/trodrigues/lxjs.org/htdocs/v2',
...
function copyFiles() {
  ...
  rmdest.on('exit', function(){
    var cp = spawn('mv', [SOURCE_PATH+'/static', PUBLIC_PATH]);
...
function build() {
  var cmd = spawn(SOURCE_PATH+'/bin/build.sh', [SOURCE_PATH, PUBLIC_PATH]);

  cmd.stdout.setEncoding('utf-8');
  cmd.stdout.on('data', function(data) { console.log(data); });

  cmd.stderr.setEncoding('utf-8');
  cmd.stderr.on('data', function(data) { console.log(data); });

  cmd.on('exit', function(code) {
    console.log('done with build', code);
    cleanup();
  });
...
function cloneRepo() {
  var clone = spawn('git', ['clone', 'http://github.com/pgte/lxjs.git', SOURCE_PATH]);
